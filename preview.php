...
if($filt == 'Processing' || $filt== 'Ready For Pickup' || $filt=='Pending'){
 ...
 <form method="post">
         <?php
         $q = "SELECT * FROM orders WHERE status='$filt'";
         $result = mysqli_query($db,$q);
         $qe = "SELECT amount FROM orders WHERE status='$filt'";
         $ttl = mysqli_query($db,$qe);

         $grp =mysqli_fetch_assoc($ttl);
         $grp1 = $grp['amount'];
         $sum =0;
         $result = mysqli_query($db,$q);
         while ($row = mysqli_fetch_assoc($ttl)) {
           foreach ($row as $row['amount'] =>$value) {
             $sum+=$value;
           }
         }
