...
$query = "SELECT payment_status FROM payment WHERE order_id=".$id;
$result = mysqli_query($db,$query);
$v = mysqli_fetch_assoc($result);

echo "Order Number: ".$id;
echo "<br>Current Status: ".$v['payment_status'];
 ...
 <br>
 <input  id="status"name="status" type="text" list="sts" placeholder="Update payment" autocomplete="off"/>
 <datalist id="sts">
...
<button name="cl" id="cls">Close</button>
</form>
</div>
...
if($up_st == 'Pending' || $up_st== 'Paid' || $up_st=='Cancelled'){
  echo $up_st;
  $id = $_COOKIE['idt'];

  $qry= "UPDATE payment SET payment_status='$up_st' WHERE order_id=".$id;
  $reslt = mysqli_query($db,$qry);
